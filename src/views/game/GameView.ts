module game {
  export class GameView extends egret.DisplayObjectContainer {
    ...
    private addMEL(): void {
      ...
      //断线重联
      GDGame.Msg.ins.addEventListener(
        room.RoomMessage.ACK_GAME_CONTINUE,
        this.onGameContinue,
        this
      );
    }
    ...
    private ACK_GAME_RESULT(evt: egret.Event): void {
      ...
      for (let i: number = 0; i < body.hulist.length; i++) {
        ...
        for (
          let j: number = 0;
          j < body.hulist[i].huInfo.score_change.length;
          j++
        ) {
      ...
      if (body.hulist.length > 1) {
        //一炮多响
        this.gameUI.playAnim(
          "ypdx",
          body.hulist[0].seat,
          body.hulist[0].huInfo.obtainsit
        );
      }
    ...
    private removeMEL(): void {
      ...
      //服务器通知客户端 单次胡牌消息
      GDGame.Msg.ins.removeEventListener(
        GameMessage.VGID_GAME_GAMERESULT,
        this.ACK_ALL_GAMERESULT,
        this
      );
